<?php 
  ...
  {
    // epharma
    if( $chkbox[$k] == "epharma" || $chkbox[$k] == "all_cat" ) {
    ...
    // chaldal
    if( $chkbox[$k] == "chldal" /*|| $chkbox[$k] == "all_cat"*/ ) {
    ...
    // techshop
    if( $chkbox[$k] == "tshop" || $chkbox[$k] == "all_cat" ) {
    ...
    // eboighor
    if( $chkbox[$k] == "book" || $chkbox[$k] == "all_cat" ) {
      $eboi_html = file_get_html('https://eboighar.com/search?term='.$search_key.'');
      $eboi_list = $eboi_html->find('.rowbg2',1);
      $eboi_name = $eboi_list->find('h3');
      $eboi_price = $eboi_list->find('.des');
      $eboi_link = $eboi_list->find('.bookimg');
      $eboi_img = $eboi_list->find('.card-img-top');
    ...
    // jadroo
    if( $chkbox[$k] == "other_cat" || $chkbox[$k] == "all_cat" || $chkbox[$k] == "dress" ) {
      $jadroo_html = file_get_html('https://www.jadroo.com/shop?s='.$search_key.'');
      $jadroo_list = $jadroo_html->find('.category-product',0);
    ...
    // othoba
    if( $chkbox[$k] == "other_cat" || $chkbox[$k] == "all_cat" || $chkbox[$k] == "book" || $chkbox[$k] == "dress" ) {
    ...
    // efair
    if( $chkbox[$k] == "other_cat" || $chkbox[$k] == "all_cat" ) {
      $efair_html = file_get_html('https://electronicsfairbd.com/index.php?route=product/search&search='.$cldl_search_key.'&limit=50');
      $efair_list = $efair_html->find('.row',4);
      $efair_name = $efair_list->find('h4');
      $efair_price = $efair_list->find('.price-tax');
      $efair_img = $efair_list->find('img');
      $efair_link = $efair_list->find('a');
    }
    // shoppersbd
    $sprbd_ok = 0;
    if( $chkbox[$k] == "other_cat" || $chkbox[$k] == "all_cat" || $chkbox[$k] == "dress" ) {
          ...
          if( $chkbox[$k] == "epharma" || $chkbox[$k] == "all_cat" ) {
            for ( $i = 0, $j = 0; $i < sizeof($epharma_name); $i++, $j+=3 ){ ?>
              ...
              <div class="col-md-3 col-sm-6 product-each">
                <img src="<?php echo $epharma_img[$i]->attr['src']; ?>" alt="">
                <p class="name"><?php echo $epharma_name[$i]->plaintext; ?></p>
                <p class="price"><?php echo $epharma_price[$i]->plaintext; ?></p>
                <button class="buy-btn"> <a href="<?php echo $epharma_link[$j]->attr['href']; ?>" target="_blank">BUY IT</a> </button>
              </div>
          ...
          if( $chkbox[$k] == "book" || $chkbox[$k] == "all_cat" ) {
            for ( $i = 0; $i < sizeof($eboi_price); $i++ ){ ?>
              ...
              <div class="col-md-3 col-sm-6 product-each">
                <img src="<?php 
                  if( $eboi_img[$i]->attr['src'] == "https://static.eboighar.com/thumbimages/mid_thumb_books/" ) {
                    echo $eboi_img[$i] = "https://eboighar.com/frontend/assets/images/default/140/no-image.png";
                  }
                  else {
                    echo $eboi_img[$i]->attr['src'];
                  }
                ?>" alt="">
                <p class="name"><?php echo $eboi_name[$i]->plaintext; ?></p>
                <p class="price"><?php echo $eboi_price2[$i]->plaintext; ?></p>
        ...
        While($k < sizeof($chkbox))
        {
          if( $chkbox[$k] == "other_cat" || $chkbox[$k] == "all_cat" ) {
            for ( $i = 0, $j = 0; $i < sizeof($efair_name); $i++, $j+=2 ) { ?>
              ...
              <div class="col-md-3 col-sm-6 product-each">
                <img src="<?php echo $efair_img[$i]->attr['src']; ?>" alt="">
                <p class="name"><?php echo $efair_name[$i]->plaintext; ?></p>
                <p class="price"><?php echo $efair_price[$i]->plaintext; ?></p>
                <button class="buy-btn"> <a href="<?php echo $efair_link[$j]->attr['href']; ?>" target="_blank">BUY IT</a> </button>
              </div>
